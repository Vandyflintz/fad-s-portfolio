...
(function ($) {
	...
	function drawExtradiv(clicked_obj) {
		//console.log(clicked_obj.attr('data-lcl-thumb'));
		setTimeout(function () {
			var lclWindow = document.getElementById('lcl_window');
	
			// Check if the lcl_window div exists
			if (lclWindow) {
				// Remove existing extra-info div if it exists
				var existingExtraInfoDiv = lclWindow.querySelector('.extra-info');
				if (existingExtraInfoDiv) {
					lclWindow.removeChild(existingExtraInfoDiv);
				}
	
				// Create and append the new extra div
				...
				if (typeof clicked_obj === 'number') {
					// Get the content div with class "content"
					var contentDiv = document.getElementById('ps-works-child');
	
					if (contentDiv) {
						// Get the div at the given index with class "elem"
						var elemDivs = contentDiv.querySelectorAll('.brick');
						if (clicked_obj >= 0 && clicked_obj < elemDivs.length) {
							var targetDiv = elemDivs[clicked_obj];
							// Get the data-lcl-thumb attribute value from the div
							thumbnailUrl = $(targetDiv).attr('data-id');
							//console.log($(targetDiv));
						}
					}
				} else {
					// Assuming clicked_obj is an element with the data-lcl-thumb attribute
					thumbnailUrl = clicked_obj.attr('data-id');
				}
				//console.log(thumbnailUrl, typeof clicked_obj, );
				// Set the content of the extra-info div
				const interactivePanel = buildInteractivePanel(thumbnailUrl);
				extraInfoDiv.appendChild(interactivePanel);
	
				// Append the extra div to the lcl_window div
				lclWindow.appendChild(extraInfoDiv);
			}
		}, 500);
	}
	
	function buildInteractivePanel(imgid) {
		// Search for the item with the matching id
		const item = imgArrayPs.find(item => item.id === imgid);
	
		if (!item) {
			console.error(`Item with id ${imgid} not found.`);
			return;
		}
	
		const iconClass = item.id_liked.includes(userid)
        ? "fas fa-thumbs-up liked"
        : "fas fa-thumbs-up unliked";
		...
		thumbsUpIcon.onclick = function () {
			...
			updateLike(thumbsUpIcon.id, imgArrayPs, likesCountSpan);
		} else {
		...
		commentsCountSpan.className = "smargin";
		commentsCountSpan.textContent = item.comment_count;
		const commentsLabelSpan = document.createElement("span");
		commentsLabelSpan.id = "nl" + item.id;
		commentsLabelSpan.className = "smargin";
		...
		ltabComments.onclick = function () {
			openCommentsDiv(commentsCountSpan.id, imgArrayPs); 
